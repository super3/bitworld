    ...
    constructor(scene, player) {
        ...
        for (let i = 0; i < scene.buildingManager.floors.length; i++) {
            ...
            */
            ...
            elevatorSprite.setInteractive({ useHandCursor: true });

            elevatorSprite.on('pointerdown', (pointer) => {
                if (pointer.rightButtonDown()) return;
                const selected = this.scene.selectedPlayer;
                if (selected) {
                    this.scene.onElevatorZoneClicked(i, selected);
                }
            });
...
continueElevatorTravel(originalTarget, direction) {
    const stepsRemaining = Math.abs(originalTarget - this.elevatorCurrentFloor);
    let stepsTaken = 0;
